...
const AssemblyDropdown = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef(null);
  const iconRef = useRef(null);

  const assemblies = ["Central", "Dzorwulu", "English", "Peniel"];

  const toggleDropdown = () => setIsOpen(!isOpen);

  const handleSelect = (assembly) => {
    onChange(assembly); // Notify the parent of the selected value
...
>
  <span>{value || "Select Assembly"}</span>
  <svg
